...
GameDemo::GameDemo()
    ...
    , m_IsFinished{ false }
{
    m_EndgameTextFont.loadFromFile("Assets\\arial.ttf"); //font
    ...
    m_EndgameSoundBuffer.loadFromFile("Assets\\Test.wav"); //sound
...
void GameDemo::Update(float deltaTime){
    ...
    if (!m_IsFinished) {
        ...
        if (m_Door.Contains(m_MainCharacter)) {
            m_EndgameSound.play(); // joue le son contenu dedans
